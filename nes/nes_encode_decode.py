...
def decode(ggcode):
    ...
    """
    if 6 != len(ggcode) != 8:
        raise ValueError("Game Genie code must be either 6 or 8 characters long")
    ggcode.upper()
    ...
    try:
        ...
        raise ValueError("Invalid Game Genie code (character(s) found not in [" + ''.join(code2letter) + "])")
    # This is from the resource mentioned in the link at the top of the file
    address = (
            0x8000 |
            ((rc[3] & 7) << 12) |
            ((rc[5] & 7) << 8) | ((rc[4] & 8) << 8) |
            ((rc[2] & 7) << 4) | ((rc[1] & 8) << 4) | (rc[4] & 7) | (rc[3] & 8))
    ...
    if not rc[2] & 0x8:
        print("warning: 3rd character is not one of [" + ''.join(code2letter[8:]) + "]")
    data = None
    comp = None
    if len(ggcode) == 6:
        # Address patch without compare (6-letter code)
        data = (((rc[1] & 7) << 4) | ((rc[0] & 8) << 4) | (rc[0] & 7) | (rc[5] & 8))
    else:
        # Address patch with compare (8-letter code)
        # (len is 8, because it can only be 6 or 8 because of the precondition
        # test).
        data = (((rc[1] & 7) << 4) | ((rc[0] & 8) << 4) | (rc[0] & 7) | (rc[7] & 8))
        comp = (((rc[7] & 7) << 4) | ((rc[6] & 8) << 4) | (rc[6] & 7) | (rc[5] & 8))
        return [address, data, comp]
...
def __boundchecks(name, value, lowerbound, upperbound):
    ...
    if not lowerbound <= value <= upperbound:
        raise ValueError("'%s' is not within 0x%X--0x%X (%s = 0x%X)" % (name, lowerbound, upperbound, name, value))
...
def encode(address, data, comp: None, altcode: False):
    ...
    give a warning.
    """
    __boundchecks(name="address",value=address,lowerbound=0x8000,upperbound=0xFFFF)
    __boundchecks(name="data", value=data, lowerbound=0x00, upperbound=0xFF)
    codes = ([0] * 6)
    ...
    c[0] = ((data >> 4) & 8) | (data & 7)
    if comp is not None:
        __boundchecks(name="comp", value=comp, lowerbound=0x00, upperbound=0xFF)
        c.extend([0, 0])
        #data = (
        #        ((rc[1] & 7) << 4) | ((rc[0] & 8) << 4) |
        #         (rc[0] & 7)       |  (rc[7] & 8)
